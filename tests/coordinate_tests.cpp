...
TEST(ShipTests, test_ship_detects_parallel_ship_overlap_different_col) {
    ...
    auto ship1 = Ship::create('a', {2,1}, Direction::DOWN);
    auto ship2 = Ship::create('a', {2,2}, Direction::DOWN);
...
TEST(ShipTests, test_ship_detects_parallel_ship_overlap_same_col) {
    auto battleship = Ship::create('b', {2,1}, Direction::DOWN);
    for (uint8_t i=0; i <10; ++i)
    {
        auto destroyer = Ship::create('d', {i,1}, Direction::DOWN);
        if (i < 1 || i >= 6)
        {
...
TEST(ShipTests, test_ship_detects_perpendicular_ship_overlap) {
    ...
    auto ship1 = Ship::create('d', {0,2}, Direction::DOWN);
    // Sail W -> E through the staionary ship at coordinate {0,2}
    for (uint8_t i=0; i <4; ++i)
    {
        auto ship2 = Ship::create('d', {0,i}, Direction::RIGHT);
        if (i < 1 || i > 2)
        {
            ASSERT_FALSE(ship1.overlaps(ship2));
            ASSERT_FALSE(ship2.overlaps(ship1));
...
TEST(ShipTests, test_ship_detects_perpendicular_ship_non_overlap) {
    ...
    auto ship1 = Ship::create('d', {2,2}, Direction::DOWN);
    ...
    auto ship3 = Ship::create('d', {2,2}, Direction::RIGHT);
    for (uint8_t i=0; i <5; ++i)
    {
        auto ship2 = Ship::create('d', {i,0}, Direction::DOWN);
